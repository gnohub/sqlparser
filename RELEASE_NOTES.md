...
`v0.2.0-dev` 是 `sqlparser` 的开发预览版本，面向 SQL 解析、结构化读取、受控改写和反解析链路。

## 主要变化

- 增加 Oracle 方言转换层，覆盖可安全映射到当前 AST 的常用 Oracle SQL。
- 增加 Oracle 示例、CLI 方言参数、批量 JSON 方言字段和完整回归矩阵。
- 保持 Oracle 公共输出形态，deparse、summary 和模型 JSON 不暴露内部 `$N`、`EXCEPT` 等转换细节。
...
- 模型 JSON schema：`sqlparser.model/v1`
- 动态库 ABI 主版本：`libsqlparser.so.0`
- vendored `libpg_query` tag：`17-6.2.2`

兼容性规则见 [兼容性策略](./doc/compatibility_policy.md)。
