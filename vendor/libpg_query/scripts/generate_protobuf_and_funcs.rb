...
class Generator
  ...
  ENUM_ZERO_DEFAULTS = ['LimitClauseStyle'].freeze
  OUTMETHOD_CONDITIONS = {
    ['SelectStmt', 'limitClauseStyle'] => 'node->limitClauseStyle != LIMIT_CLAUSE_STYLE_DEFAULT',
  }
  PROTOBUF_FIELD_NUMBER_OVERRIDES = {
    ['SelectStmt', 'startWithClause'] => 21,
    ['SelectStmt', 'connectByClause'] => 22,
    ['SelectStmt', 'connectByNoCycle'] => 23,
    ['SelectStmt', 'connectByFirst'] => 24,
    ['SelectStmt', 'limitClauseStyle'] => 25,
  }
  OUTNAME_OVERRIDES = {
  ...
  def generate_outmethods!
    ...
    ['nodes/parsenodes', 'nodes/primnodes'].each do |group|
      @struct_defs[group].each do |node_type, struct_def|
        ...
        struct_def['fields'].each do |field_def|
          ...
          outname_json = name
          protobuf_field_number = PROTOBUF_FIELD_NUMBER_OVERRIDES[[node_type, name]]
          if protobuf_field_number
            sequential_protobuf_field_count = protobuf_field_count
            protobuf_field_count = protobuf_field_number
          end
          ...
          # Abstract types that have no read or out methods.
            ...
            outmethod_condition = OUTMETHOD_CONDITIONS[[node_type, name]]
            if outmethod_condition
              @outmethods[node_type] += format("  if (%s) {\n", outmethod_condition)
          ...
          protobuf_field_count = sequential_protobuf_field_count if protobuf_field_number
        end
      end
    end
    ...
    ['nodes/parsenodes', 'nodes/primnodes', 'nodes/nodes', 'nodes/lockoptions'].each do |group|
      @enum_defs[group].each do |enum_type, enum_def|
        ...
        if ENUM_ZERO_DEFAULTS.include?(enum_type)
          protobuf_field = 0
        else
...
"
    ...
    File.write('./src/include/pg_query_outfuncs_defs.c',
    ...
    File.write('./src/include/pg_query_readfuncs_defs.c',
      ("// This file is autogenerated by ./scripts/generate_protobuf_and_funcs.rb\n\n" +
       read_defs + "\n\n" + read_impls).rstrip + "\n")
