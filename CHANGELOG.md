...
- 修正 SQL Server、Oracle、达梦及对应兼容入口的反斜杠字符串和 `N` 前缀输出，覆盖整句 SQL、片段读取和 `source_selector` 复制。
- 修复 MySQL 及其兼容入口的字符串字面量转义，避免改写时传入的字符串值改变或生成无效 SQL。
- 公开 API、公开结构体布局、View JSON 字段和所有权规则不变。

### 用例与验证

- 新增覆盖 13 个方言入口的 2,276 组字符串回归组合，检查输出文本、语义值、完整 View、批内顺序和失败回滚。
- 完整 `make test` 通过。
...
- 减少 INSERT 值复制、Oracle 系及达梦 `INSERT ALL/FIRST` 值替换、函数表达式及参数修改、UPDATE 赋值与参数替换混合批次中的重复整句解析与原文扫描。
- 合并可连续处理的修改，按需同步解析状态，保持修改顺序、`source_selector` 取值语义、整批失败回滚和原有资源上限。公开 API、公开结构体布局、View JSON 字段和所有权规则不变。
...
- 扩充覆盖 13 个方言入口的批量 patch 回归与独立性能回放，核对顺序取值、索引变化、混合修改、bind、注释、资源限制和失败回滚。
- 完整 `make test` 和 ABI 检查通过，公开导出符号保持 162 个；批量 patch 回归的 Valgrind 检查未报内存错误或泄漏。
