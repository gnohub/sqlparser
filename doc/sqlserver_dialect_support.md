...
- `@name` 和 `?` 参数在 deparse 和 SQL View JSON 中保持公共形态，不输出内部 `$1`、`$2`。
...
- SQL View JSON 中可归属的表达式片段使用公共 SQL Server 形态。
...
当前 SQL Server 基础矩阵包含 56 条用例：41 条支持路径，15 条明确不支持路径。官方 `HOOK_ONLY` 覆盖矩阵包含 235 条用例。
