...
剔除 `REFERENCE_ONLY` 后，官方可实现条目为 890 条。其中当前已覆盖 186 条，未覆盖 704 条。

| 未覆盖分类 | 条目数 | 占未覆盖 704 条 |
| --- | ---: | ---: |
| `HOOK_ONLY` | 235 | 33.38% |
| `MIXED_MODEL` | 82 | 11.65% |
| `MODEL_REQUIRED` | 387 | 54.97% |

如果按“完整官方语法是否需要扩展自有 SQL Server 模型”统计，`MIXED_MODEL + MODEL_REQUIRED` 为 469 条，占未覆盖条目的 66.62%。只依赖现有 AST 和 hook 即可补齐的为 235 条，占 33.38%。
...
SQL Server 剩余未覆盖条目中，严格不能只靠现有 AST 解决的为 387 条，占 54.97%。另有 82 条属于基础形态可 hook、完整官方语法需要专用模型的混合项。按完整官方语法覆盖口径计算，需要 SQL Server 专用模型的条目合计 469 条，占 66.62%。
