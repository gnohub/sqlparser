...
- `vastbase-mysql` 的 261 条用例均为 `status = "final"`，合计 836 个独立 patch。项目兼容入口合同支持 MySQL 多目标多表 `UPDATE` 的 JOIN 链与逗号 relation 列表，每个 assignment 目标字段分别关联其写入 relation；多表形态不接受 `ORDER BY` 或 `LIMIT`。该口径来自项目可执行矩阵，不声称 Vastbase 服务端官网定义了同一语法范围。
