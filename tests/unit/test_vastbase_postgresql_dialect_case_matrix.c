...
#define SQLPARSER_VASTBASE_POSTGRESQL_CASE_FIXTURE_PATH "./tests/cases/vastbase_postgresql_dialect_input.json"
...
static int verify_success_case(const char *case_id, const char *case_name, const char *sql, json_t *expect_root)
{
	...
	if (sqlparser_handle_dialect(handle) != SQLPARSER_DIALECT_VASTBASE_POSTGRESQL) {
		sqlparser_handle_destroy(handle);
...
int main(void)
{
	...
	root = json_load_file(SQLPARSER_VASTBASE_POSTGRESQL_CASE_FIXTURE_PATH, 0, &error);
	if (root == NULL) {
		fprintf(stderr, "FAIL: unable to load fixture %s: %s\n", SQLPARSER_VASTBASE_POSTGRESQL_CASE_FIXTURE_PATH, error.text);
		return 1;
	}
