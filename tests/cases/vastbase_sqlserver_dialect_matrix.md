...
可执行夹具：`tests/cases/vastbase_sqlserver_dialect_input.json`。单元测试会逐条验证解析、View JSON、反解析输出和明确不支持语法返回码。
...
| `VSU001` | `vastbase-sqlserver-top-percent-unsupported` | SELECT TOP (10) PERCENT [id] FROM [dbo].[users] | 明确不支持 |
| `VSU002` | `vastbase-sqlserver-top-with-ties-unsupported` | SELECT TOP (10) WITH TIES [id] FROM [dbo].[users] ORDER BY [score] | 明确不支持 |
| `VSU003` | `vastbase-sqlserver-output-unsupported` | INSERT INTO [dbo].[users] ([id]) OUTPUT inserted.[id] VALUES (1) | 明确不支持 |
| `VSU004` | `vastbase-sqlserver-table-hint-unsupported` | SELECT [id] FROM [dbo].[users] WITH (NOLOCK) | 明确不支持 |
...
| `VSU007` | `vastbase-sqlserver-for-json-unsupported` | SELECT [id] FROM [dbo].[users] FOR JSON PATH | 明确不支持 |
| `VSU008` | `vastbase-sqlserver-query-hint-unsupported` | SELECT [id] FROM [dbo].[users] OPTION (RECOMPILE) | 明确不支持 |
...
| `VSU018` | `vastbase-sqlserver-nested-top-unsupported` | SELECT [id] FROM (SELECT TOP (2) [id] FROM [dbo].[users]) AS [u] | 明确不支持 |
