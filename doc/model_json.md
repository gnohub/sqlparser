...
- patch 和完整模型导入均为原子操作；任一修改失败时，原 `handle` 保持不变。
- 完整模型导入只应用相对当前模型发生变化的 selector，避免同一 AST 节点在多个视图中重复回放。
- SQL 表达式替换会在普通字段和字面量修改之后提交，使后续 selector 尽量基于原始结构定位。
...
- 单个 patch 条目只使用一种改写形式
- 完整模型编辑后重新导入时保持 `selector` 不变
