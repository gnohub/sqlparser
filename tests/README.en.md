...
`tests/unit/test_string_literal_surface.c` exercises library APIs for string values, complete SQL, expression fragments, and source copies. It is discovered automatically by `make test`. Its 2,276 combinations cover all 13 dialect entries. Failures return a nonzero exit status.
