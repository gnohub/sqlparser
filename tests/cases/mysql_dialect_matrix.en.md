...
The following six cases cover `query_graph.expressions[]` for predicate right-hand sides. Predicates link the RHS through `right_expression`; `values[]` retains existing entries but does not duplicate an expression index, and no placeholder value is synthesized when the RHS had no existing value. Every source SQL and all 23 patched SQL texts are validated byte for byte.

| Case ID | Case Name | Statement Shape | Validation Focus |
| --- | --- | --- | --- |
| M269 | `mysql-predicate-expression-function-argument-kinds` | WHERE `LIKE CONCAT(literal, bind, field, LOWER(bind))` with inter-argument comments | root-first/DFS expression order, four inline argument kinds, a nested function, argument value selectors, bind positions 1/2, and five root/argument patches |
| M270 | `mysql-predicate-expression-on-having-order` | JOIN ON `COALESCE` plus HAVING `UPPER` | multi-predicate root order, preservation of existing root-value indexes, field/bind/literal arguments, global bind positions 1/2, and three patches |
| M271 | `mysql-predicate-expression-root-argument-replace` | WHERE `COALESCE(bind, field)` | independent whole-expression and bind/field argument replacement, including an empty occurrence list after bind removal; three patches |
| M272 | `mysql-predicate-expression-argument-list-mutations` | `CONCAT(?,?,?)`, `FUNC()`, and `FUNC(?)` | head/middle/tail insertion and deletion, zero arguments and deletion to zero, and cross-statement bind renumbering; eight patches |
| M273 | `mysql-predicate-expression-opaque-boundary` | an operator containing a function plus a CASE RHS | opaque expressions expose only whole SQL/selector, without expanded function arguments or new internal-value ownership; two whole-expression patches |
| M274 | `mysql-predicate-expression-reverse-rhs-function` | `? = UPPER(field)` | retains the left bind value and `right_field` while linking the RHS function through `right_expression=0`; one root patch and one field-argument patch |
