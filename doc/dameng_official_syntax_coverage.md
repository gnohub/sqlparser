...
| `MODEL_REQUIRED` | 需要达梦专用模型，通常涉及层级查询、表变换、DMSQL 程序单元、返回宿主变量或闪回。 |
...
剔除 `REFERENCE_ONLY` 后，官方可实现语法组为 38 组。其中当前已覆盖 31 组，未覆盖 7 组。
...
达梦当前已覆盖常用查询、DML、DDL、事务、权限、`SET SCHEMA`、代表性会话参数设置语句和远程对象引用基础形态。其余 7 个语法组依赖达梦专属查询模型或程序单元语义，当前不纳入 PostgreSQL 兼容转换。
