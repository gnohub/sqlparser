...
- 外部化改写接口：`selector`、View JSON、structured patch
...
View 层用于把语句树按需导出为可供外部程序消费的结构化视图。
...
公共 ABI 不暴露 `PgQuery*` 或 PostgreSQL 节点结构，扩展点包括：
...
- View JSON 手册见 [view_json.md](./view_json.md)
