...
以下 final 用例定义项目 `vastbase-oracle` 兼容入口合同，不声称 Vastbase 服务端官网定义了相同语法范围。

| ID | 用例 | 状态 | 独立 patch | 验证重点 |
| --- | --- | --- | ---: | --- |
| `VO222` | `vastbase-oracle-merge-omitted-insert-column-value-independent` | final | 3 | 省略目标列清单时输出 `target_list_selector`；分别验证仅增加目标列并物化清单、仅增加 VALUES cell 且保持清单省略，以及替换已有 cell |
