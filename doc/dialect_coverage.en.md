...
| Dameng | `tests/cases/dameng_dialect_input.json` | 89 | 12 | 101 | 88.12% |
| Vastbase PostgreSQL mode | `tests/cases/vastbase_postgresql_dialect_input.json` | 136 | 1 | 137 | 99.27% |
| Vastbase MySQL mode | `tests/cases/vastbase_mysql_dialect_input.json` | 85 | 18 | 103 | 82.52% |
| Vastbase Oracle mode | `tests/cases/vastbase_oracle_dialect_input.json` | 131 | 16 | 147 | 89.12% |
| Vastbase SQL Server mode | `tests/cases/vastbase_sqlserver_dialect_input.json` | 330 | 15 | 345 | 95.65% |
...
- MySQL, Oracle, SQL Server, Dameng, and Vastbase compatibility-mode unsupported cases are database-specific semantics that cannot be safely represented without extending the shared AST.
